/*
* Console ASCII Ray Tracing
* https://liuliqiang.com/51/3drt_ascii.c
* by liuliqiang 2025-03-29
*/
...
{
    samplePixel();
    const uint8_t r = sample.x * 0xff;
    const uint8_t g = sample.y * 0xff;
    const uint8_t b = sample.z * 0xff;
    drawpixel(X, Y, r, g, b);
    X++;
    if (X == LCD_W)
    {
